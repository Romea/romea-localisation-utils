#ifndef __LocalisationUpdaterInterface_HPP__
#define __LocalisationUpdaterInterface_HPP__

#include <romea_common_utils/qos.hpp>

#include "localisation_parameters.hpp"
#include "localisation_updater_interface_base.hpp"
...
public :
  ...
  LocalisationUpdaterInterface(std::shared_ptr<rclcpp::Node> node,
  ...
  virtual bool heartbeat_callback(const Duration & duration) override;

  virtual DiagnosticReport get_report() override;
...
//-----------------------------------------------------------------------------
template <typename Filter_, typename Updater_, typename Msg>
LocalisationUpdaterInterface<Filter_,Updater_,Msg>::
LocalisationUpdaterInterface(std::shared_ptr<rclcpp::Node> node,
                             const std::string & topic_name):
  ...
  sub_()
{
  ...
  auto callback = std::bind(&LocalisationUpdaterInterface::process_message,
...
//-----------------------------------------------------------------------------
template <typename Filter_, typename Updater_, typename Msg>
void LocalisationUpdaterInterface<Filter_,Updater_,Msg>::
load_updater(std::unique_ptr<Updater> updater)
{
...
template <typename Filter_, typename Updater_, typename Msg>
void LocalisationUpdaterInterface<Filter_,Updater_,Msg>::
register_filter(std::shared_ptr<Filter> filter)
{
  filter_ = filter;
}
...
void LocalisationUpdaterInterface <Filter_,Updater_,Msg>::
process_message(typename Msg::ConstSharedPtr msg)
{
...
//-----------------------------------------------------------------------------
template <class Filter_, class Updater_, class Msg>
bool LocalisationUpdaterInterface <Filter_,Updater_,Msg>::
heartbeat_callback(const Duration & duration)
{
...
std::unique_ptr<UpdaterInterface> make_updater_interface(std::shared_ptr<rclcpp::Node> node,
