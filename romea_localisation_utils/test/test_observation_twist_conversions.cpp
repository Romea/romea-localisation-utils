...
TEST_F(TestObsTwistConversion, fromRomeato_ros_msg)
{
  ...
  EXPECT_DOUBLE_EQ(romea_obs_twist_msg.observation_twist.twist.linear_speeds.x,
                   romea_obs_twist.Y(romea::ObservationTwist::LINEAR_SPEED_X_BODY));
  EXPECT_DOUBLE_EQ(romea_obs_twist_msg.observation_twist.twist.linear_speeds.y,
                   romea_obs_twist.Y(romea::ObservationTwist::LINEAR_SPEED_Y_BODY));
  EXPECT_DOUBLE_EQ(romea_obs_twist_msg.observation_twist.twist.angular_speed,
                   romea_obs_twist.Y(romea::ObservationTwist::ANGULAR_SPEED_Z_BODY));
  EXPECT_DOUBLE_EQ(romea_obs_twist_msg.observation_twist.level_arm.x,
                   romea_obs_twist.levelArm.x());
  EXPECT_DOUBLE_EQ(romea_obs_twist_msg.observation_twist.level_arm.y,
                   romea_obs_twist.levelArm.y());
  EXPECT_DOUBLE_EQ(romea_obs_twist_msg.observation_twist.level_arm.z,
...
TEST_F(TestObsTwistConversion, fromRosMsgtoObs)
{
  ...
  EXPECT_DOUBLE_EQ(romea_obs_twist_bis.Y(romea::ObservationTwist::LINEAR_SPEED_Y_BODY),
                   romea_obs_twist.Y(romea::ObservationTwist::LINEAR_SPEED_Y_BODY));
  EXPECT_DOUBLE_EQ(romea_obs_twist_bis.Y(romea::ObservationTwist::ANGULAR_SPEED_Z_BODY),
                   romea_obs_twist.Y(romea::ObservationTwist::ANGULAR_SPEED_Z_BODY));
...
TEST_F(TestPoseConversion, fromRomeato_ros_msg)
{
  EXPECT_EQ(romea::extract_time(romea_obs_twist_msg).nanoseconds(),stamp.nanoseconds());
  EXPECT_STREQ(romea_obs_twist_msg.header.frame_id.c_str(),frame_id.c_str());
  EXPECT_DOUBLE_EQ(romea_obs_twist_msg.observation_twist.twist.linear_speeds.x,romea_twist.linearSpeeds.x());
  EXPECT_DOUBLE_EQ(romea_obs_twist_msg.observation_twist.twist.linear_speeds.y,romea_twist.linearSpeeds.y());
  EXPECT_DOUBLE_EQ(romea_obs_twist_msg.observation_twist.twist.angular_speed,romea_twist.angularSpeed);
