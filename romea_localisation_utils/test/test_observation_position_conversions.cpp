...
TEST_F(TestObsPositionConversion, fromRomeato_ros_msg)
{
  ...
  EXPECT_DOUBLE_EQ(romea_obs_position_msg.observation_position.position.x,
                   romea_obs_position.Y(romea::ObservationPosition::POSITION_X));
  EXPECT_DOUBLE_EQ(romea_obs_position_msg.observation_position.position.y,
...
TEST_F(TestObsPositionConversion, fromRosMsgtoObs)
{
  ...
  romea::extractObs(romea_obs_position_msg,romea_obs_position_bis);
  EXPECT_DOUBLE_EQ(romea_obs_position_bis.Y(romea::ObservationPosition::POSITION_X),
                   romea_obs_position.Y(romea::ObservationPosition::POSITION_X));
  EXPECT_DOUBLE_EQ(romea_obs_position_bis.Y(romea::ObservationPosition::POSITION_Y),
                   romea_obs_position.Y(romea::ObservationPosition::POSITION_Y));
...
TEST_F(TestPositionConversion, fromRomeato_ros_msg)
{
  EXPECT_EQ(romea::extract_time(romea_obs_position_msg).nanoseconds(),stamp.nanoseconds());
  EXPECT_STREQ(romea_obs_position_msg.header.frame_id.c_str(),frame_id.c_str());
  EXPECT_DOUBLE_EQ(romea_obs_position_msg.observation_position.position.x,romea_position.position.x());
  EXPECT_DOUBLE_EQ(romea_obs_position_msg.observation_position.position.y,romea_position.position.y());
  isSame(romea_obs_position_msg.observation_position.position.covariance,romea_position.covariance);
}
